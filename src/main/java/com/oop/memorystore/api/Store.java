...
public interface Store<V> extends Collection<V> {
  ...
  /**
   * Query an index by name and lookup the given key. This method is the equivalent of calling
   * {@link #getIndex(String)} and then {@link Index#get(Object)}
   *
  ...
  /**
   * Query an index by name and lookup the given key. This method is the equivalent of calling
   * {@link #getIndex(String)} and then {@link Index#getFirst(Object)}
   *
   * @param indexName name of the index to query
   * @param key key to lookup
  ...
  /**
   * Query an index by name and lookup the given key. This method is the equivalent of calling
   * {@link #getIndex(String)} and then {@link Index#findFirst(Object)}
   *
   * @param indexName name of the index to query
   * @param key key to lookup
  ...
  /**
   * Find index with name. This is the same as {@link Store#findIndex(String)}, but returns a null
   * instead of an optional if an index cannot be found.
   *
  ...
  /**
   * Find index with name. This is the same as {@link Store#getIndex(String)}, but returns an
   * optional instead of a null if an index cannot be found.
   *
   * @param indexName name of index to lookup
  ...
  /**
   * Clear the existing indexes and reindex the entire store. This can be a slow operation depending
   * on the number of items in the store and total number of indexes.
   *
  ...
  /**
   * Reindex a collection of items. This method will need to be called anytime a change is made to
   * items stored within the store that causes its indexes to become out of date.
   *
  ...
  /**
   * Reindex a particular item. This method will need to be called anytime a change is made to an
   * item stored within the store that causes its indexes to become out of date.
   *
  ...
  /**
   * Adds an item to the store and indexes it. If the item already exists in the store, it will be
   * reindexed.
   *
  ...
  /**
   * Adds all item to the store and indexes them. If an item already exists in the store, it will be
   * reindexed.
   *
  ...
  /**
   * Create a copy of this store. This can be an expensive operation depending on the number of
   * items and indexes present. The copied store will be fully independent from this store. Any
   * changes made to the copy will not reflect back onto this store.
   *
   * @return copy
   */
  Store<V> copy();
  ...
  /**
   * Returns an unmodifiable view of this store. This method allows modules to provide users with
   * "read-only" access to internal collections. Query operations on the returned store "read
   * through" the backed store, and attempts to modify the returned store, whether direct or via its
   * iterator, result in an <tt>UnsupportedOperationException</tt>.
   *
   * <p>
   *
  ...
  /**
   * Returns a synchronized (thread-safe) map Store backed by this store. In order to guarantee
   * serial access, it is critical that <strong>all</strong> access to the backing store is
   * accomplished through the returned store.
   *
